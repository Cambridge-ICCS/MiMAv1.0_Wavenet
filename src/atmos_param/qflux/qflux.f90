...
real ::    warmpool_k     = 1,    & ! wave number of warmpool []
	   ...
	   trop_atlantic_amp  = 0. , &  ! amplitude of tropical atlantic perturbation [W/m2]
           spcz_weight    = 0.  ! amplitude of tropical atlantic perturbation [W/m2]
...
!
    integer i,j
    real lon,lat,piphase, pigulfphase, latgulf, latspcz
...
!    piphase = warmpool_phase/pi
    ...
    do j=1,size(latb)-1
       ...
       latgulf = (lat-37)/10
       latspcz= (lat-warmpool_centr+15)/warmpool_width
       ...
       if( abs(lat) .le. 1.0 ) then
		...
		if( lon .ge. (warmpool_phase - 54)*pi/180. .AND. lon .lt. (warmpool_phase + 162)*pi/180.) then !modified by cig, nov 15 2017
                  flux(i,j) = flux(i,j) &
                     &+ (1.-lat**4.)*(1-spcz_weight)*warmpool_amp*cos(warmpool_k*(lon-piphase))
       ...
       if( abs(latspcz) .le. 1.0 .and. warmpool_localization_choice == 2) then
          do i=1,size(lonb)-1
             lon = 0.5*(lonb(i+1) + lonb(i))
 	     	if( lon .ge. (warmpool_phase - 34)*pi/180. .AND. lon .lt. (warmpool_phase + 182)*pi/180.) then !modified by cig, nov 6 2018
                  flux(i,j) = flux(i,j) &
                     &+ (1.-latspcz**4.)*warmpool_amp*spcz_weight*cos(warmpool_k*(lon-piphase-20*pi/180))
	        endif 	
	   !  write (*,*) "lon",lon," phiphase",piphase
          enddo
       endif
